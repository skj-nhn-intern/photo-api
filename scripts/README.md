...
| 4 | `sudo ./scripts/4-setup-telegraf.sh` | Telegraf 설치, 설정: `/opt/telegraf/telegraf.conf` |

## 사전 조건

- Ubuntu 22.04 LTS (또는 호환 배포)
- 스크립트 실행 시 **프로젝트 루트**(`app/main.py`, `requirements.txt`, `conf/promtail-config.yaml`, `conf/telegraf.conf`)가 현재 머신에 있어야 함  
  - 다른 경로에 있으면 `PHOTO_API_SOURCE`, `PROMTAIL_CONF_SOURCE`, `TELEGRAF_CONF_SOURCE` 등으로 지정
...
| `TELEGRAF_VERSION` | Telegraf 버전 (기본: 1.37.1) |
| `LOKI_URL` | Promtail → Loki 주소 (예: http://loki:3100, 이미지/배포 환경에 맞게 설정) |
| `INFLUX_URL`, `INFLUX_TOKEN` | Telegraf → InfluxDB 주소/토큰 (배포 환경에 맞게 설정) |

## 설치 결과

- **photo-api**: `/opt/photo-api`, systemd 서비스 `photo-api`, 포트 8000
- **Promtail**: `/opt/promtail/`, 설정 `/opt/promtail/promtail-config.yaml`
- **Telegraf**: `/opt/telegraf/`, 설정 `/opt/telegraf/telegraf.conf`
